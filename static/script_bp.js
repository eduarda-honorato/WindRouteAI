...
function desenharGrafoPesos(nos, grafo_com_pesos, caminho, posicoes) {
    ...
    grafo_com_pesos.forEach((viz, i) => {
        viz.forEach(v => {
            const from = nos[i];
            const to = v[0];     // destino (ex: "T2")
            const peso = v[1];   // peso (ex: 221)
            ...
            if (!addedEdges.has(edgeKey)) {
                edgesArr.push({
                    from,
                    to,
                    label: peso.toString(),     // mostra o peso na tela
                    font: {
                    size: 12,
                    color: "black",
                    face: "arial",
...
document.addEventListener('DOMContentLoaded', function() {
    loadTheme();
    carregarGrafoPesosInicial();
});

// função para carregar o grafo inicial sem caminho
function carregarGrafoPesosInicial() {
    fetch('/graph-data-bp')
    .then(res => res.json())
    .then(data => {
        const container = document.querySelector(".map-container");
        ...
        desenharGrafoPesos(data.nos, data.grafo_com_pesos, [], data.posicoes);
    })
    .catch(err => console.error("Erro ao carregar grafo com pesos inicial:", err));
}